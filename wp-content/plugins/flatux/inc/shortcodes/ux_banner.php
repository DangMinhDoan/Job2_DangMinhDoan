<?php
// [flatux_banner]
function flatsome_flatux_banner( $atts, $content = null ){
   ...
   /* Banner Effects */
   if($effect) wp_enqueue_style( 'flatux-effect');
   ...
   if($bg_gradient){
		$css_gradient = array(
			array('attribute' => 'background','value' => 'linear-gradient(to '.$bg_gradient_to.', '.$bg_color.', '.$bg_gradient.')',),
			array('attribute' => 'height', 'value' => '100%',),
		);
	}	
  ...
  <div class="banner <?php echo $classes; ?>" id="<?php echo $_id; ?>">
     ...
     <div class="banner-inner fill">
        <div class="banner-bg fill" <?php echo $parallax; ?>>
            ...
            <?php if($bg_gradient) echo '<div '.get_shortcode_inline_css( $css_gradient ).'></div>' ?>
            <?php if($bg_overlay) echo '<div class="overlay"></div>' ?>
            <?php require( get_template_directory() . '/inc/shortcodes/commons/border.php' ) ;?>
			<?php if($effect) echo '<div class="effect-'.$effect.' bg-effect fill no-click"><ul><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li></ul></div>'; ?>
        </div>
        ...
        <div class="banner-layers <?php if($container_width !== 'full-width') echo 'container'; ?>">
            ...
            <?php
            // Get Layers
            if (!get_theme_mod('flatsome_fallback', 1) || (has_shortcode( $content, 'flatux_text_box' ) || has_shortcode( $content, 'ux_hotspot' ) || has_shortcode( $content, 'flatux_image' ))) {
              ...
              $depth = '';
              if($text_bg) $depth = '1';
              //echo do_shortcode( '[flatux_text_box text_align="'.$text_align.'" parallax="'.$parallax_text.'" animate="'.$animation.'" depth="'.$depth.'" padding="'.$padding.'" bg="'.$text_bg.'" text_color="'.$text_color.'" width="'.intval($text_width).'" width__sm="60%" position_y="'.$y.'" position_x="'.$x.'"]'.$content.'[/flatux_text_box]' );
			  echo do_shortcode( $content );
            } ?>
        </div>
      </div>
...
if($effect == "sliding"){ ?>
<style>
<?php echo '#'.$_id; ?> .effect-sliding li:nth-of-type(1),<?php echo '#'.$_id; ?> .effect-sliding li:nth-of-type(2),<?php echo '#'.$_id; ?> .effect-sliding li:nth-of-type(3) {
  background-image: linear-gradient(-60deg, <?php echo $bg_color; ?> 50%, <?php echo $bg_gradient; ?> 50%);
}
</style>
<?php }
...
add_shortcode('flatux_banner', 'flatsome_flatux_banner');